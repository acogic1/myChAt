...
public class RequestFragment extends Fragment {
    ...
    private DatabaseReference FriendsRequestsDatabaseReference;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        FriendsRequestsDatabaseReference=FirebaseDatabase.getInstance().getReference().child("Friend_Requests");
    ...
    public void onStart() {
        ...
        FirebaseRecyclerAdapter<Requests,RequestViewHolder> firebaseRecyclerAdapter
            ...
            protected void populateViewHolder(final RequestViewHolder viewHolder, Requests model, int position) {
                final String lists_users_id=getRef(position).getKey();
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        if(dataSnapshot.exists()){
                            ...
                            if(request_type.equals("received")){
                                UsersReference.child(lists_users_id).addValueEventListener(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                                ...
                                                builder.setItems(options, new DialogInterface.OnClickListener() {
                                                    ...
                                                    public void onClick(DialogInterface dialogInterface, int position) {
                                                        if(position==0){
                                                            ...
                                                            FriendsDatabaseReference.child(online_user_id).child(lists_users_id).child("date").setValue(saveCurrentDate)
                                                                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                                                                        @Override
                                                                        public void onSuccess(Void aVoid) {
                                                                            FriendsDatabaseReference.child(lists_users_id).child(online_user_id).child("date").setValue(saveCurrentDate)
                                                                                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                                                                                        @Override
                                                                                        public void onSuccess(Void aVoid) {
                                                                                            FriendsRequestsDatabaseReference.child(online_user_id).child(lists_users_id).removeValue()
                                                                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                                        @Override
                                                                                                        public void onComplete(@NonNull Task<Void> task) {
                                                                                                            if(task.isSuccessful()){
                                                                                                                FriendsRequestsDatabaseReference.child(lists_users_id).child(online_user_id).removeValue()
                                                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                                                            @Override
                                                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                                                if(task.isSuccessful()){
                                                                                                                                    Toast.makeText(getContext(),"Friend Req accept sucessifull",Toast.LENGTH_LONG).show();
                                                                                                                                }
                                                                                                                            }
                                                                                                                        });
                                                                                                            }
                                                                            ...
                                                                            if(task.isSuccessful()){
                                                                                FriendsRequestsDatabaseReference.child(lists_users_id).child(online_user_id).removeValue()
                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                            ...
                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                if(task.isSuccessful()){
                                                                                                    Toast.makeText(getContext(),"Friend Req cancel successiful",Toast.LENGTH_LONG).show();
                                                                                                }
                                                                                            }
                                                                                        });
                                                                            }
